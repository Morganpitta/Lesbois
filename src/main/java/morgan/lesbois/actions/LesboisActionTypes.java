package morgan.lesbois.actions;
...
import morgan.lesbois.actions.bientity.PossessBiEntityActionType;
import morgan.lesbois.actions.entity.*;

public class LesboisActionTypes {
    ...
    public static <T extends EntityActionType> ActionConfiguration<T> registerEntityAction(String path, TypedDataObjectFactory<T> dataFactory) {
        ActionConfiguration<T> configuration = ActionConfiguration.of(Lesbois.id(path), dataFactory);
